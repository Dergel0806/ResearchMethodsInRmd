...
plotBarChart <- function(d, 
  ...
  rownames(d) <- 'value'
  d <- data.frame(t(d))
  ggplot(d, aes(x = rownames(d), y = value)) + 
    theme_bw() + 
...
As it can be seen, most of the albums had approximately same median song length (except for LIB and YSB which have bigger values).  
...
In terms of time evolution we may assume that Beatles started from very ranged songs, but went down to smaller range of length. In the end they increased range of their songs again.
...
Since IQR is also connected to range, our previous assumption is corroborated (in terms of time evolution we may assume that Beatles started from very ranged songs, but went down to smaller range of length, in the end they increased range of their songs again)  
